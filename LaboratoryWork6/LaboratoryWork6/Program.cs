    ...
    internal class Program
    {
        ...
        private static void ShowRoots(double[][] datas)
        {
            ...
            rootsB[rootsB.Length - 1] =
                (datas[1][datas[1].Length - 1] - datas[1][datas[1].Length - 2]) / GetH(datas, rootsB.Length) -
                GetH(datas, rootsB.Length) * 2 * rootsC[rootsC.Length - 1] / 3;
            var rootsD = new double[rootsB.Length];
            for (int i = 0; i < rootsD.Length - 1; i++)
                rootsD[i] = (rootsC[i + 1] - rootsC[i]) / (3 * GetH(datas, i + 1));
            rootsD[rootsD.Length - 1] = -rootsC[rootsC.Length - 1] / (3 * GetH(datas, rootsD.Length));
